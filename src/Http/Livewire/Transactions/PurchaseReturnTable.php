...
class PurchaseReturnTable extends DataTable
{
    ...
    public function columns(): array
    {
        return [
            Column::make('Number', 'return_number')->searchable()->sortable()
                ->view('inventory::livewire.partials.purchase-return-table.number'),
            Column::make('Supplier', 'supplier.name')->relation('supplier'),
            Column::make('Warehouse', 'warehouse.name')->relation('warehouse'),
            Column::make('Status', 'status')->badge('neutral', StatusBadge::colors()),
            Column::make('Action')
                ->view('inventory::livewire.partials.purchase-return-table.actions'),
        ];
    }

    public function query(): Builder
    {
        return PurchaseReturn::query()->with(['supplier', 'warehouse', 'purchaseOrder']);
    }
}
