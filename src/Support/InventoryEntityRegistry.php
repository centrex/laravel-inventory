...
class InventoryEntityRegistry
{
    public static function entities(): array
    {
        return [
            ...
            'suppliers' => [
                ...
                'form_fields'   => [
                    self::field('code', 'text', ['required', 'string', 'max:30'], null, null, null, null, 'Code (auto-generated if blank)'),
                    self::field('name', 'text', ['required', 'string', 'max:300']),
                    ...
                    self::field('meta', 'json', ['nullable', 'array'], []),
                    self::virtualField('create_user', 'checkbox', ['boolean'], false, 'Create login user account'),
                    self::virtualField('user_password', 'password', ['required', 'string', 'min:8', 'required_if:create_user,true'], null, 'Login password'),
                ],
            ],
            'customers' => [
                'label'         => 'Customers',
                ...
                'form_fields'   => [
                    ...
                    self::field('meta', 'json', ['nullable', 'array'], []),
                    self::virtualField('create_user', 'checkbox', ['boolean'], false, 'Create login user account'),
                    self::virtualField('user_password', 'password', ['nullable', 'string', 'min:8', 'required_if:create_user,true'], null, 'Login password'),
                ],
            ],
    ...
    public static function validationRules(string $entity, ?Model $record = null, array $payload = []): array
    {
        ...
        if ($record === null && !empty($payload['create_user']) && ($emailField = EntityUserProvisioner::emailField($entity)) !== null) {
            $rules[$emailField] = array_values(array_filter(
                [...$rules[$emailField], 'required', Rule::unique('users', 'email')],
    ...
    public static function fillablePayload(string $entity, array $payload, bool $forCreate = false): array
    {
        $definition = self::definition($entity);
        $output = [];
        ...
        if ($forCreate && array_key_exists('code', $output) && ($output['code'] === null || $output['code'] === '')) {
            $output['code'] = self::autoGeneratedCode($entity) ?? $output['code'];
        }
    ...
    /**
     * Next sequential code for entities that support auto-generation
     * (e.g. CUS-00007, SUP-00002). Returns null for other entities.
     */
    public static function autoGeneratedCode(string $entity): ?string
    {
        $prefix = ['customers' => 'CUS', 'suppliers' => 'SUP'][$entity] ?? null;
        ...
        $model = self::makeModel($entity);
        $next = (int) $model->newQuery()->max($model->getKeyName()) + 1;
    ...
    /**
     * Names of form fields that are validated but never persisted on the
     * entity model (e.g. the create-login-user controls).
     *
     * @return array<int, string>
     */
    public static function virtualFieldNames(string $entity): array
    {
        return collect(self::definition($entity)['form_fields'])
            ->filter(fn (array $field): bool => $field['virtual'] ?? false)
            ->pluck('name')
            ->all();
    }
    ...
    /** A form field that is validated but excluded from the model payload. */
    private static function virtualField(string $name, string $type, array $rules, mixed $default = null, ?string $label = null): array
    {
        return [...self::field($name, $type, $rules, $default, label: $label), 'virtual' => true];
    }
