...
<x-tallui-card title="Lines" subtitle="Products returned into stock." icon="o-queue-list" :shadow="true">
    <div class="overflow-x-auto">
        <table class="table table-sm w-full">
            ...
            <tbody>@foreach($record->items as $item)<tr><td>{{ $item->product?->name ?? 'Product' }}</td><td>{{ number_format((float) $item->qty_returned, 4) }}</td><td>{{ number_format((float) $item->unit_price_amount, 2) }}</td><td>{{ number_format((float) $item->unit_cost_amount, 2) }}</td><td>{{ number_format((float) $item->line_total_amount, 2) }}</td></tr>@endforeach</tbody>
        </table>
    </div>
</x-tallui-card>
</div>
