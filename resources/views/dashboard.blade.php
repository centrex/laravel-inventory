<x-layouts::app>
    ...
    <x-tallui-page-header title="Inventory"
        subtitle="Stock, pricing, warehouses, vendors, customers, and order workflows." icon="o-building-storefront">
        <x-slot:actions>
            ...
            @endcan
            @can('inventory.purchase-orders.view')
                <x-tallui-button label="Requisition" icon="o-clipboard-document-check" :link="route('inventory.requisitions.index')"
            ...
            @endcan
            @can('inventory.sale-orders.view')
                <x-tallui-button label="Quotation" icon="o-document-duplicate" :link="route('inventory.quotations.index')"
                    class="btn-outline btn-sm" />
            @endcan
            ...
            @if (Route::has('inventory.expenses.index'))
                @can('inventory.expenses.view')
                    <x-tallui-button label="Expenses" icon="o-receipt-percent" :link="route('inventory.expenses.index')"
                        class="btn-outline btn-sm" />
                @endcan
            @endif
            @if ($canViewForecast)
    ...
    <div x-data="{
        ...
        toggle() { this.open = !this.open;
            localStorage.setItem('inv_quick_actions', this.open ? 'true' : 'false'); }
    }">
        <div class="mb-3 flex items-center justify-between">
            <span class="text-base-content/40 text-xs font-semibold uppercase tracking-widest">Quick Actions</span>
            <button @click="toggle()" class="btn btn-ghost btn-xs text-base-content/50 hover:text-base-content gap-1">
                <span x-text="open ? 'Hide' : 'Show'"></span>
                <x-heroicon-o-chevron-up x-show="open" class="h-3 w-3" x-cloak />
                <x-heroicon-o-chevron-down x-show="!open" class="h-3 w-3" x-cloak />
            </button>
        </div>
        <div x-show="open" x-transition:enter="transition ease-out duration-150"
            ...
            <div class="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4 lg:grid-cols-8">
                @if (Route::has('payroll.entities.employees.index'))
                    <a href="{{ route('payroll.entities.employees.index') }}"
                        class="border-base-200 bg-base-100 hover:bg-base-200 flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 text-center transition">
                        <x-heroicon-o-identification class="text-primary h-7 w-7" />
                ...
                @can('inventory.purchase-orders.create')
                    <a href="{{ route('inventory.purchase-orders.create') }}"
                        class="border-base-200 bg-base-100 hover:bg-base-200 flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 text-center transition">
                        <x-heroicon-o-arrow-down-tray class="text-primary h-7 w-7" />
                ...
                @can('inventory.purchase-orders.create')
                    <a href="{{ route('inventory.requisitions.create') }}"
                        class="border-base-200 bg-base-100 hover:bg-base-200 flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 text-center transition">
                        <x-heroicon-o-clipboard-document-check class="text-warning h-7 w-7" />
                ...
                @can('inventory.sale-orders.create')
                    <a href="{{ route('inventory.sale-orders.create') }}"
                        class="border-base-200 bg-base-100 hover:bg-base-200 flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 text-center transition">
                        <x-heroicon-o-shopping-cart class="text-success h-7 w-7" />
                ...
                @can('inventory.sale-orders.create')
                    <a href="{{ route('inventory.quotations.create') }}"
                        class="border-base-200 bg-base-100 hover:bg-base-200 flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 text-center transition">
                        <x-heroicon-o-document-duplicate class="text-info h-7 w-7" />
                ...
                @can('inventory.channels.checkout')
                    <a href="{{ route('inventory.pos.index') }}" target="_blank"
                        class="border-base-200 bg-base-100 hover:bg-base-200 flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 text-center transition">
                        <x-heroicon-o-device-phone-mobile class="text-secondary h-7 w-7" />
                ...
                @can('inventory.transfers.view')
                    <a href="{{ route('inventory.transfers.index') }}"
                        class="border-base-200 bg-base-100 hover:bg-base-200 flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 text-center transition">
                        <x-heroicon-o-arrows-right-left class="text-info h-7 w-7" />
                ...
                @can('inventory.shipments.view')
                    <a href="{{ route('inventory.shipments.index') }}"
                        class="border-base-200 bg-base-100 hover:bg-base-200 flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 text-center transition">
                        <x-heroicon-o-paper-airplane class="text-primary h-7 w-7" />
                ...
                @can('inventory.master-data.view')
                    <a href="{{ route('inventory.entities.warehouse-products.index') }}"
                        class="border-base-200 bg-base-100 hover:bg-base-200 flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 text-center transition">
                        <x-heroicon-o-cube class="text-success h-7 w-7" />
                ...
                @if ($canViewForecast)
                    <a href="{{ route('inventory.reports.index') }}"
                        class="border-base-200 bg-base-100 hover:bg-base-200 flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 text-center transition">
                        <x-heroicon-o-chart-bar class="text-secondary h-7 w-7" />
                        <span class="text-sm font-medium">Reports</span>
                    </a>
                    <a href="{{ route('inventory.reports.customer-heatmap') }}"
                        class="border-base-200 bg-base-100 hover:bg-base-200 flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 text-center transition">
                        <x-heroicon-o-map class="text-accent h-7 w-7" />
                ...
                @can('inventory.adjustments.view')
                    <a href="{{ route('inventory.adjustments.index') }}"
                        class="border-base-200 bg-base-100 hover:bg-base-200 flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 text-center transition">
                        <x-heroicon-o-scale class="text-warning h-7 w-7" />
                ...
                @can('inventory.adjustments.create')
                    <a href="{{ route('inventory.adjustments.create') }}"
                        class="border-base-200 bg-base-100 hover:bg-base-200 flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 text-center transition">
                        <x-heroicon-o-plus-circle class="text-warning h-7 w-7" />
                ...
                @if (Route::has('payroll.entries.index'))
                    <a href="{{ route('payroll.entries.index') }}"
                        class="border-base-200 bg-base-100 hover:bg-base-200 flex cursor-pointer flex-col items-center gap-2 rounded-2xl border p-4 text-center transition">
                        <x-heroicon-o-users class="text-accent h-7 w-7" />
    ...
    <x-tallui-tab :tabs="[
    ...
    ]" :active="request('dashboard_tab', 'overview')" variant="bordered" size="sm" class="mb-2">
        <x-slot:overview>
            {{-- <div class="w-full mb-6 shadow stats">
            <x-tallui-stat
        ...
        <x-slot:forecast>
            ...
            @else
                <x-tallui-card title="Forecast Access Required"
                    subtitle="Forecasting is available to users with the inventory reports permission."
                    icon="o-lock-closed" :shadow="true" class="mb-6">
                    <x-tallui-empty-state title="No forecast access"
                        ...
                        icon="o-shield-exclamation" size="sm" />
                </x-tallui-card>
            @endif
        </x-slot:forecast>
        ...
        <x-slot:target>
            @if ($canViewForecast)
                ...
                <form method="GET" action="{{ route('inventory.dashboard') }}" class="mb-6">
                    ...
                    <x-tallui-card title="Sales Target Inputs"
                        subtitle="Tune the target period, gross margin, net profit, growth, and expense allocation."
                        icon="o-adjustments-horizontal" :shadow="true">
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-6">
                            <label class="form-control">
                                <span class="label-text text-xs font-semibold">Lookback Days</span>
                                <input type="number" min="7" max="730" name="target_lookback_days"
                                    value="{{ request('target_lookback_days', 90) }}"
                                    class="input input-bordered input-sm">
                            </label>
                            <label class="form-control">
                                <span class="label-text text-xs font-semibold">Target Days</span>
                                <input type="number" min="1" max="366" name="target_days"
                                    value="{{ request('target_days', 30) }}" class="input input-bordered input-sm">
                            </label>
                            <label class="form-control">
                                <span class="label-text text-xs font-semibold">Gross Margin %</span>
                                <input type="number" min="1" max="95" step="0.01"
                                    name="target_gross_margin_pct"
                                    value="{{ request('target_gross_margin_pct', 25) }}"
                                    class="input input-bordered input-sm">
                            </label>
                            <label class="form-control">
                                <span class="label-text text-xs font-semibold">Net Profit %</span>
                                <input type="number" min="0" max="80" step="0.01"
                                    name="target_net_margin_pct" value="{{ request('target_net_margin_pct', 10) }}"
                                    class="input input-bordered input-sm">
                            </label>
                            <label class="form-control">
                                <span class="label-text text-xs font-semibold">Growth %</span>
                                <input type="number" min="0" max="200" step="0.01"
                                    name="target_growth_pct" value="{{ request('target_growth_pct', 0) }}"
                                    class="input input-bordered input-sm">
                            </label>
                            <label class="form-control">
                                <span class="label-text text-xs font-semibold">Expense Allocation %</span>
                                <input type="number" min="0" max="100" step="0.01"
                                    name="target_expense_allocation_pct"
                                    value="{{ request('target_expense_allocation_pct', 100) }}"
                                    class="input input-bordered input-sm">
                            </label>
                        </div>
                        <div class="mt-4 flex justify-end">
                            <x-tallui-button type="submit" label="Recalculate" icon="o-calculator"
            ...
            @else
                <x-tallui-card title="Sales Target Access Required"
                    subtitle="Sales targets are available to users with the inventory reports permission."
                    icon="o-lock-closed" :shadow="true" class="mb-6">
                    <x-tallui-empty-state title="No target access"
    ...
    <x-tallui-card title="Master Data" subtitle="Open CRUD screens for inventory master tables." icon="o-squares-2x2"
        :shadow="true">
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            @can('inventory.master-data.view')
                @foreach ($entities as $entity => $definition)
                    <a href="{{ route("inventory.entities.{$entity}.index") }}"
                        class="border-base-200 bg-base-100 hover:border-primary hover:bg-base-200 group flex flex-col gap-1 rounded-xl border p-4 transition">
                        <div class="mb-1 flex items-center justify-between">
                            <x-heroicon-o-folder
                                class="text-base-content/40 group-hover:text-primary h-5 w-5 transition" />
                            <x-heroicon-o-arrow-top-right-on-square
                                class="text-base-content/30 group-hover:text-primary h-4 w-4 transition" />
                        </div>
                        <span
                            class="text-base-content text-sm font-semibold leading-tight">{{ $definition['label'] }}</span>
                        <span class="text-base-content/50 text-xs">Manage records</span>
                    </a>
                @endforeach
            @endcan
            ...
            @if (Route::has('accounting.expenses'))
                <a href="{{ route('accounting.expenses') }}"
                    ...
                    <span class="text-base-content text-sm font-semibold leading-tight">Expenses</span>
                    <span class="text-base-content/50 text-xs">Track spend</span>
                </a>
            @endif
        </div>
    </x-tallui-card>
